...
public class GoblinCouncil {
    ...
    // Armazena novos goblins criados durante formação do império
    private List<com.rpggame.entities.Goblin> newEmpireGoblins = new ArrayList<>();
    ...
    /**
     * Forma império escolhendo apenas uma família dominante
     * com exatamente 8 membros + 1 líder
     */
    private void formEmpire(List<GoblinFamily> families) {
        if (families.isEmpty())
            return;

        // Limpar lista de novos goblins
        newEmpireGoblins.clear();

        // Escolher a família mais forte para se tornar o império
        ...
        if (empire == null) {
            empire = families.get(0);
        }

        // Eliminar todos os membros exceto o líder
        List<com.rpggame.entities.Goblin> members = new java.util.ArrayList<>(empire.getMembers());
        for (com.rpggame.entities.Goblin goblin : members) {
            if (goblin.getPersonality() != com.rpggame.entities.GoblinPersonality.LEADER) {
                goblin.takeDamage(99999); // Eliminar membros não-líderes
            }
        }

        // Agora só resta o líder - adicionar 8 novos membros ao império
        for (int i = 0; i < 8; i++) {
            // Criar novo goblin perto da cabana do império
            // Alternando entre personalidades (COMMON e AGGRESSIVE)
            com.rpggame.entities.GoblinPersonality personality = (i % 2 == 0)
                    ? com.rpggame.entities.GoblinPersonality.COMMON
                    : com.rpggame.entities.GoblinPersonality.AGGRESSIVE;

            com.rpggame.entities.Goblin newMember = new com.rpggame.entities.Goblin(
                    empire.getHutPosition().x + random.nextInt(200) - 100,
                    empire.getHutPosition().y + random.nextInt(200) - 100,
                    personality);
            newMember.setFamily(empire);
            empire.addMember(newMember);
            newEmpireGoblins.add(newMember); // Armazenar para adicionar à lista de inimigos
        }

        // Renomear para império
        empire.setFamilyName("IMPÉRIO GOBLIN");

        System.out.println("👑 Império formado com 1 Líder + 8 Membros!");
        System.out.println("   Família escolhida: " + empire.getFamilyName());
        System.out.println("   Total de goblins: " + empire.getMembers().size());
    }
    ...
    /**
     * Retorna e limpa a lista de novos goblins criados durante formação do império
     */
    public List<com.rpggame.entities.Goblin> getAndClearNewEmpireGoblins() {
        List<com.rpggame.entities.Goblin> goblins = new ArrayList<>(newEmpireGoblins);
        newEmpireGoblins.clear();
        return goblins;
    }

    /**
     * Desfaz o império goblin quando a família imperial é derrotada
     */
    public void dissolveEmpire() {
        if (currentDecision == CouncilDecision.GOBLIN_EMPIRE) {
            System.out.println("\n💥 ===== IMPÉRIO GOBLIN DESTRUÍDO =====");
            System.out.println("👑 O Império Goblin foi derrotado pelo jogador!");
            System.out.println("🏛️ O conselho foi dissolvido...");
            System.out.println("=====================================\n");
